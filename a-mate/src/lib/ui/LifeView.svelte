<script lang="ts">
  ...
  import { LIFE_GRID, isInsideLife, occupiedWorldCells, placementOrigin, rotatedOffsets, rotatedOrigin, rotatedSize, spriteGroundAnchor, wallOccupiedIndices, wallPlacementOrigin } from '../interior/geometry';
  ...
  async function clickFloor(x:number,y:number){if(editing){if(category!=='window')place(x,y);return}if(occupied.has(`${x},${y}`))return;try{await lifeMoveCell(x,y);await poll()}catch{msg('이동할 수 없는 자리예요.')}}
...
<div class="lifewrap"><div class="title"><b>{life.owner_name}님의 방</b>{#if me.life_id!==me.my_life_id}<span>방문 중</span>{/if}<small>{life.occupants.length}명</small>{#if flash}<em>{flash}</em>{/if}{#if isOwner&&!editing}<button onclick={beginEdit}>인테리어 설정</button>{/if}</div>
  <div class="scene">
    <svg viewBox={`0 0 ${VW} ${VH}`} aria-label="아이소메트릭 미니라이프">
    ...
    {#if ghost}{@const gp=objectLayout(ghost)}<div class="furniture ghost" class:invalid={!canPlace(ghost)} style:left={`${gp.x/VW*100}%`} style:top={`${gp.y/VH*100}%`} style:width={`${gp.w/VW*100}%`} style:transform={`translate(${-gp.anchor[0]*100}%,${-gp.anchor[1]*100}%)`} style:z-index={gp.z}><FurnitureSprite assetId={ghost.asset_id} rotation={spriteRotation(ghost)} life/></div>{/if}
...
<style>
  ...
  .scene{position:relative;width:min(100%,900px);aspect-ratio:900/460;margin:auto;overflow:hidden;border-radius:14px;background:linear-gradient(#fbf8ee,#e9e4d8);box-shadow:var(--shadow-soft)}svg{position:absolute;inset:0;width:100%;height:100%}.north,.west{stroke:#a69483;stroke-width:2}.tile{cursor:pointer;stroke-width:.65;transition:filter .1s}.tile:hover{filter:brightness(1.12)}.tile.taken{cursor:not-allowed}.ghost-cell,.ghost-wall-cell{fill:rgba(68,211,159,.38);stroke:#24a77a;stroke-width:1.2;pointer-events:none}.ghost-cell.blocked,.ghost-wall-cell.blocked{fill:rgba(226,80,80,.4);stroke:#c93636}.placed-cell,.placed-wall-cell{fill:rgba(44,166,224,.08);stroke:#2ca6e0;stroke-width:.85;pointer-events:none}.ground-point{fill:#ff5b5b;stroke:white;stroke-width:.8;pointer-events:none}.wallslot{fill:rgba(116,225,197,.06);stroke:rgba(124,111,208,.22);stroke-width:.7;cursor:crosshair;transition:fill .1s}.wallslot:hover,.wallslot.hovered{fill:rgba(116,225,197,.16);stroke:#58cdb0;stroke-width:1}
