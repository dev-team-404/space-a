<script lang="ts">
  ...
  import { THEME_BACKGROUND_BY_ID } from '../interior/theme-art';
  ...
  const themeBackground=$derived(THEME_BACKGROUND_BY_ID[theme.id] ?? '');
...
<div class="lifewrap" bind:this={root}>{#if flash}<div class="title"><em>{flash}</em></div>{/if}
  <div class="scene" class:moving bind:this={scene}>
    ...
    {#if themeBackground}<img class="theme-background" src={themeBackground} alt="" />{/if}
    <svg viewBox={`${VIEW_X} ${VIEW_Y} ${VIEW_W} ${VIEW_H}`} aria-label="아이소메트릭 미니라이프">
      <polygon class="north" fill={themeBackground?'transparent':theme.wall} points={points([[OX,VIEW_Y],[VIEW_X+VIEW_W,VIEW_Y],b,a])}/>
      <polygon class="west" fill={themeBackground?'transparent':theme.wallSide} points={points([[VIEW_X,VIEW_Y],[OX,VIEW_Y],a,d])}/>
      <polygon fill={themeBackground?'transparent':theme.floorBase} points={points([a,b,d])}/>
      {#each Array(GRID_H+1) as _,y}{#each Array(GRID_W+1) as _,x}
        {#if isFloorCell([x,y])}
          <polygon class="tile" class:image-theme={!!themeBackground} class:taken={occupied.has(`${x},${y}`)} fill={themeBackground?'transparent':((x+y)%2?theme.floorAlt:theme.floorBase)} stroke={themeBackground&&editing?'rgba(80,70,60,.28)':themeBackground?'transparent':theme.grout} points={tile(x,y)} onmouseenter={()=>hoverCell=[x,y]} onclick={()=>clickFloor(x,y)}/>
        {:else if x+y===LIFE_FLOOR.maxXYExclusive}
          <polygon class="tile floor-cut" fill={themeBackground?'transparent':((x+y)%2?theme.floorAlt:theme.floorBase)} stroke={themeBackground&&editing?'rgba(80,70,60,.28)':themeBackground?'transparent':theme.grout} points={tile(x,y)}/>
        {/if}
      {/each}{/each}
      {#if !themeBackground}
  ...
  {#if editing&&draft}<section class="editor"><header><div><b>인테리어 설정</b></div><nav><button onclick={cancel}>취소</button><button class="primary" onclick={save} disabled={saving}>{saving?'저장 중…':'저장'}</button></nav></header>
    ...
    {#if editorTab==='design'}
      <div class="themes">{#each themeItems as t}<button class:active={draft.wallpaper===t.wallpaper} onclick={()=>chooseTheme(t.wallpaper,t.floor)}><span>{#if THEME_BACKGROUND_BY_ID[t.id]}<img src={THEME_BACKGROUND_BY_ID[t.id]} alt="" />{:else}<i style:background={t.wall}></i><i style:background={t.floorBase}></i>{/if}</span><small>{t.name}</small></button>{/each}</div>
...
<style>
  ...
  .theme-background{position:absolute;inset:0;width:100%;height:100%;object-fit:fill;image-rendering:pixelated;pointer-events:none}
  .scene{position:relative;width:min(100%,640px);aspect-ratio:560/257;margin:auto;overflow:hidden;contain:layout paint;isolation:isolate;transform:translateZ(0);background:linear-gradient(#fbf8ee,#e9e4d8)}.edit-trigger{position:absolute;left:10px;top:10px;z-index:1100;width:34px;height:34px;display:grid;place-items:center;padding:0;border:1px solid var(--line);border-radius:50%;background:var(--frame-bg);color:var(--ink-soft);box-shadow:var(--shadow-soft);cursor:pointer;opacity:0;pointer-events:none;transition:opacity .15s ease,color .15s ease,border-color .15s ease}.scene:hover .edit-trigger,.scene:focus-within .edit-trigger{opacity:1;pointer-events:auto}.edit-trigger:hover{color:var(--accent-strong);border-color:var(--accent)}.edit-trigger svg{position:static;width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round}.scene>svg{position:absolute;inset:0;width:100%;height:100%}.room-edge{fill:none;stroke:#a69483;stroke-width:1;vector-effect:non-scaling-stroke}.tile{cursor:pointer;stroke-width:.65;transition:filter .1s,fill .1s}.tile:hover{filter:brightness(1.12)}.tile.image-theme:hover{fill:rgba(116,225,197,.3);stroke:rgba(36,167,122,.95);stroke-width:1;filter:none}.tile.taken{cursor:not-allowed}.tile.floor-cut{pointer-events:none;cursor:default}.scene.moving .tile{cursor:progress}.ghost-cell,.ghost-wall-cell{fill:rgba(68,211,159,.38);stroke:#24a77a;stroke-width:1.2;pointer-events:none}.ghost-cell.blocked,.ghost-wall-cell.blocked{fill:rgba(226,80,80,.4);stroke:#c93636}.placed-cell{fill:rgba(44,166,224,.08);stroke:#2ca6e0;stroke-width:.85;pointer-events:none}.ground-point{fill:#ff5b5b;stroke:white;stroke-width:.8;pointer-events:none}.wallslot{fill:transparent;stroke:transparent;stroke-width:.7;cursor:crosshair;transition:fill .1s}.wallslot:hover,.wallslot.hovered{fill:rgba(116,225,197,.16);stroke:#58cdb0;stroke-width:1}
