<script lang="ts">
  ...
  let editing=$state(false),saving=$state(false),moving=$state(false),error=$state(''),flash=$state(''),category=$state<FurnitureCategory>('sofa'),selectedId=$state(FURNITURE[0].id),rotation=$state<Rotation>(0),wall=$state<WallSide>('north');
    ...
    <svg viewBox={`${VIEW_X} ${VIEW_Y} ${VIEW_W} ${VIEW_H}`} aria-label="아이소메트릭 미니라이프">
      ...
      {#if editing}
        {#each activeDesign.objects as placed,placedIndex (`footprint-${placedIndex}`)}
          ...
          {:else if placed.wall==='north'||placed.wall==='west'}
            {#each wallOccupiedIndices(placed.cell[0],placed.size[0]) as wallIndex}
              <polygon class="placed-wall-cell" points={wallTile(placed.wall,wallIndex)}/>
            {/each}
          {/if}
        {/each}
      {/if}
