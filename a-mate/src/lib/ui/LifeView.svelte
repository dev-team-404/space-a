<script lang="ts">
  ...
  const VIEW_X=160,VIEW_Y=15,VIEW_W=580,VIEW_H=420;
...
<div class="lifewrap" bind:this={root}>{#if me.life_id!==me.my_life_id||flash}<div class="title">{#if me.life_id!==me.my_life_id}<span>방문 중</span>{/if}{#if flash}<em>{flash}</em>{/if}</div>{/if}
...
<style>
  .lifewrap{display:flex;flex-direction:column;gap:8px}.title{display:flex;align-items:center;gap:8px;font-size:12px}.title span{background:#ffd9ca;border-radius:99px;padding:2px 8px}.title small{color:var(--ink-soft)}.title em{font-style:normal;color:var(--accent)}button{font:inherit;color:inherit}
  .scene{position:relative;width:min(100%,900px);aspect-ratio:580/420;margin:auto;overflow:hidden;contain:layout paint;isolation:isolate;transform:translateZ(0);border-radius:14px;background:linear-gradient(#fbf8ee,#e9e4d8);box-shadow:var(--shadow-soft)}.edit-trigger{position:absolute;left:10px;top:10px;z-index:1100;width:34px;height:34px;display:grid;place-items:center;padding:0;border:1px solid var(--line);border-radius:50%;background:var(--frame-bg);color:var(--ink-soft);box-shadow:var(--shadow-soft);cursor:pointer}.edit-trigger:hover{color:var(--accent-strong);border-color:var(--accent)}.edit-trigger svg{position:static;width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round}.scene>svg{position:absolute;inset:0;width:100%;height:100%}.north,.west{stroke:#a69483;stroke-width:2}.tile{cursor:pointer;stroke-width:.65;transition:filter .1s}.tile:hover{filter:brightness(1.12)}.tile.taken{cursor:not-allowed}.scene.moving .tile{cursor:progress}.ghost-cell,.ghost-wall-cell{fill:rgba(68,211,159,.38);stroke:#24a77a;stroke-width:1.2;pointer-events:none}.ghost-cell.blocked,.ghost-wall-cell.blocked{fill:rgba(226,80,80,.4);stroke:#c93636}.placed-cell{fill:rgba(44,166,224,.08);stroke:#2ca6e0;stroke-width:.85;pointer-events:none}.ground-point{fill:#ff5b5b;stroke:white;stroke-width:.8;pointer-events:none}.wallslot{fill:transparent;stroke:transparent;stroke-width:.7;cursor:crosshair;transition:fill .1s}.wallslot:hover,.wallslot.hovered{fill:rgba(116,225,197,.16);stroke:#58cdb0;stroke-width:1}
  ...
  .agent-bubble{position:absolute;left:50%;bottom:94%;transform:translateX(-50%);box-sizing:border-box;width:max-content;min-width:76px;max-width:160px;padding:6px 10px;background:#fffdf8;color:#2b2520;border:1.5px solid #6f665b;border-radius:11px;font:600 11px/1.35 'Segoe UI','Malgun Gothic',sans-serif;text-align:center;white-space:pre-wrap;overflow-wrap:anywhere;box-shadow:0 2px 6px #342d2438}.agent-bubble:after{content:'';position:absolute;left:50%;bottom:-5px;width:8px;height:8px;background:#fffdf8;border-right:1.5px solid #6f665b;border-bottom:1.5px solid #6f665b;transform:translateX(-50%) rotate(45deg)}
