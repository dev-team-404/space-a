<script lang="ts">
  ...
  let { assetId, rotation = 0, label = '', life = false }: { assetId: string; rotation?: Rotation; label?: string; life?: boolean } = $props();
...
<style>
  ...
  .sprite:not(.life) img{transform:matrix(1,var(--shear-y),0,var(--scale-y),0,0);transform-origin:50% 50%}
  .sprite:not(.life).mirrored img{transform:matrix(1,var(--shear-y),0,var(--scale-y),0,0) scaleX(-1)}
  .sprite.life{display:block;height:auto;transform:matrix(1,var(--shear-y),0,var(--scale-y),0,0);transform-origin:0 0}
  .sprite.life img{width:100%;height:auto;max-width:none;max-height:none;object-fit:initial}
