...
{#each threads as t (t.entry.entry_id)}
  <article>
    <header>{#if showOwnerAvatar(t.entry,meId,isOwner)}{#if ownSprite}<img class="ava" src={'data:image/png;base64,'+ownSprite} alt=""/>{:else}<span class="ava">🤖</span>{/if}{/if}<b>{t.entry.author_name}</b><time>{new Date(t.entry.created_at).toLocaleString()}</time>
      <span class="acts">
    ...
    <div class="replies">
      {#each t.replies as reply (reply.entry_id)}
        <article class="reply">
          <header>{#if showOwnerAvatar(reply,meId,isOwner)}{#if ownSprite}<img class="ava" src={'data:image/png;base64,'+ownSprite} alt=""/>{:else}<span class="ava">🤖</span>{/if}{/if}<b>{reply.author_name}</b><time>{new Date(reply.created_at).toLocaleString()}</time>
...
<style>section{padding:16px;display:flex;flex-direction:column;gap:12px}form{display:flex;gap:8px}input{flex:1;padding:9px;border:1px solid var(--pastel-lav);border-radius:8px}button{border:0;border-radius:8px;padding:7px 11px;background:var(--accent);color:var(--accent-ink);cursor:pointer}.list{display:grid;gap:8px}.list article{padding:11px 13px;background:var(--pastel-cream);border-radius:10px}.list header{display:flex;gap:8px;align-items:center;font-size:11px}.list .ava{width:18px;height:18px;border-radius:50%;object-fit:cover;vertical-align:middle;margin-right:2px}.list time{color:var(--ink-soft)}.list .acts{margin-left:auto;display:flex;gap:4px}.list header button{padding:3px 7px;background:var(--pastel-lav);color:var(--ink)}.list p{margin:7px 0 0}.replies{margin-top:8px;display:grid;gap:6px;border-left:2px solid var(--pastel-lav);padding-left:10px}.list .reply{padding:8px 10px;background:transparent;border:1px solid var(--pastel-lav);border-radius:8px}</style>
