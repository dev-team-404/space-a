...
import { FURNITURE_BY_ID, MIRRORED_DIRECTION, OPPOSITE_DIRECTION, ROTATIONS, SPRITE_DIRECTION_BY_ROTATION, wallRotation, type SpriteSource } from './catalog';
...
describe('interior asset orientation contract', () => {
  ...
  it('derives the only valid window rotation from its wall', () => {
    ...
    const window = FURNITURE_BY_ID.get('window.mint-square')!;
    expect(window.sprites[90]).toBe(window.sprites[180]);
    expect(window.render.mirrorX[90]).toBe(true);
    expect(window.render.mirrorX[180]).toBe(false);
  });
