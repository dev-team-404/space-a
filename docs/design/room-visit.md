...
- 각 PNG의 접지 앵커와 투영 너비는 자산 매니페스트에 기록한다. 현재 PNG의 기본 앵커는
  하단 중앙 `[0.5, 1]`이지만 렌더러가 이를 상수로 추측하지 않는다. 이미지 높이는 원본
  비율을 유지하며 정사각형 렌더 박스로 제한하지 않는다.
...
- 카탈로그는 선택된 원본의 접지 `anchor=[x,y]`를 가진다. anchor는 이미지의 실제 하단
  접점이며 논리 footprint의 화면 전면 꼭짓점에 고정한다.
