...
1. "나" 탭 → **"봇"**. 개인정보 "이름" → **"마스코트 이름"**, **"호칭"** 필드 신설(기본 `"주인"`).
2. 호칭(`owner_title`)을 **일기·코칭·채팅·오늘의 한마디·잡담·메모리** 전반에 전파(하드코딩 "주인" 대체).
3. **MBTI 저장 시 자동 재생성 제거** + MBTI 필드 옆 안내문.
4. **"마스코트 생성" 섹션 신설**(개인정보와 주인 메모리 사이): 미리보기 + 재생성 + 저장. 저장을 눌러야만 전체 반영. 연결 탭의 "캐릭터 재생성" 버튼 삭제.
...
**의미 전환의 결과(명시)**: `user_name`은 계속 a-hub 점유자·방 주인 이름으로 쓰이므로, 이제 **Life 방/미니홈피에 마스코트 이름이 표시**된다("나=봇" 전환의 자연스러운 귀결). 사람의 실명은 더 이상 수집·표시하지 않는다.

## A. "봇" 탭 UI (`groups.ts`, `MeGroup.svelte`)

- `groups.ts`: `me` 라벨 `'나' → '봇'`.
- 개인정보 필드: **마스코트 이름**(`user_name`) · **호칭**(`owner_title`, 신설) · 조직 · 아이디 · MBTI.
...
- 섹션 순서: **개인정보 → 마스코트 생성(신규 §B) → 주인 메모리**.
