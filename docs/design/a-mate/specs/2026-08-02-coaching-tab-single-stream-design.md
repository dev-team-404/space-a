...
> 반영돼 있다 — **다시 조사하지 말 것.** §3의 실측은 원래 `dd4a5bc`(#152) 기준이라 여섯 항목이 낡아 있었고,
...
| PR | 내용 | 의존 |
|----|------|------|
| **A (Rust)** | `first_seen` serde 노출 + `content_first_seen` 보존 테이블 | ③·⑥ 머지 완료 → **착수 가능** |
| **B (프론트)** | 단일 스트림·3분류·라인 색·탭 배지·홈 위젯 | A + §4 미결 질문 해소 |
