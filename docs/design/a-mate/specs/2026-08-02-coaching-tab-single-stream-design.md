...
- 활성 집합은 **상한(§2.2)을 적용하기 전의 활성 항목 전부**다. 고정 슬롯 항목도 자연히 들어간다(그것도 `status='new'`이므로). 상한 밖으로 밀린 항목까지 "봤다"고 치는 셈인데, 화면에 보이는 것만 세려면 고정 슬롯 판정(`pinAcks`·`today`)을 배지 계산부로 끌어와야 해서 배보다 배꼽이 커진다. 잃는 것도 작다 — 나중에 상한 안으로 올라와도 배지에 한 번 안 잡힐 뿐이다.
