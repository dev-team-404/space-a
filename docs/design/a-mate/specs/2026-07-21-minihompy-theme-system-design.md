...
CSS(`theme.css`)는 `:root[data-skin="sky"][data-theme="light"] { … }` … 8개 블록으로 §5 값을 정의. `data-theme`은 항상 light/dark로 확정되므로 CSS에 `@media` 중복 불필요(단일 소스).
...
## 8. 설정 UI (`Settings.svelte`)

기존 설정 창 상단에 **테마** 섹션 한 블록 추가.

- **테마 모드**: 세그먼트 컨트롤 `[라이트] [다크] [시스템]`.
- **색상 세트**: 스킨 스와치 선택 `하늘 · 민트 · 살구 · 라벤더`(각 accent 색 원형, 선택 표시).
- 변경 즉시 `setTheme()` 호출 → 저장 + 세 창 반영. 별도 "저장" 버튼 불필요.
- 카피: 라벨 "테마", "색상 세트". 선택 즉시 반영되므로 상태 토스트 불필요.
...
| 파일 | 처리 |
|------|------|
| `src/lib/theme.css` | 토큰 계약 + 8개 스킨×모드 블록 + 상태색으로 **전면 재작성** |
| `src/App.svelte` | `--bg-grad`/`--ink`/`--frame-2`/`--panel2`/`--pastel-lav` 치환, `#0b3327`→`--accent-ink`, `#3a1512`→`--coral-ink` |
| `src/Settings.svelte` | 구 토큰 치환, `#fff`(primary 버튼)→`--accent-ink`, 테마 섹션 UI 추가 |
| `src/Mascot.svelte` | 컨텍스트 메뉴 `#fff`→`--accent-ink`, 구 토큰 치환, theme.ts 초기화 |
| `src/lib/ui/CoachTab.svelte` | **`#f4f1fa` ×3 → `--surface-inset`(+ `--text`)** [버그 수정], `--pastel-*` 치환 |
| `src/lib/ui/SessionModal.svelte` | `#fdf9ef44`→`--cream`(알파 정리), `#eef7f344`→`--accent-tint` |
| `src/lib/ui/LifeView.svelte` | `#ffd9ca`→`--coral`(제목 배지), `color:white`→`--accent-ink`, 구 토큰 치환 |
| `src/lib/ui/DiaryTab.svelte` | `.sel color:#fff`·`.dot #fff`→`--accent-ink` |
| `src/lib/ui/home/TipCard.svelte` | `#0b3327`→`--accent-ink` |
| 기타 `src/lib/ui/**` | `var(--ink*/--frame*/--panel2/--pastel*/--mint-tint*)` 잔여 치환(전수 grep) |
...
| 회귀 가드 | `src/**/*.svelte` 스타일에 **하드코딩 hex 색 없음**을 검사하는 테스트/스크립트(theme.css·스프라이트 예외) → 저대비 버그 재발 방지 |
...
1. **토큰 기반** — `theme.css` 재작성(계약 + 8블록 + 상태색), `theme.ts` 추가, 회귀 가드 테스트.
2. **백엔드** — `theme_get`/`theme_set` 커맨드 + `theme:changed` emit + Rust 테스트.
3. **컴포넌트 마이그레이션** — §9 파일 전수 치환(버그 수정 포함), 하드코딩 hex 제거.
4. **설정 UI + 창 연동** — `Settings.svelte` 테마 섹션, 세 창 theme.ts 초기화·구독.
