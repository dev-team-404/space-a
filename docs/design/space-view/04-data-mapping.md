...
| `knowledge[].visibility` | 권한 | `org` = 로비·방 밖에서도 원문까지 / `space` = 비멤버에겐 제목만 (아래 "지식 잠금") |
...
- **지식 잠금 (2026-07-14 시나리오 실험)**: `visibility:'space'` 문서는 서버가 비멤버에게
  title만 내려준다(body·summary 없음). 표시 결정 — 로비 신착 목록에 **🔒 + 제목으로
  노출**한다(숨기지 않음: "이런 지식이 있구나 → 공유 요청"의 발견 가능성이 순환에 유리).
  모달은 잠금 안내 + 인용 횟수만, 공유 요청 안내 문구를 곁들인다.
