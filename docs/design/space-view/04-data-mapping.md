...
- ⚠️ **발견한 계약↔설계 충돌**: space-view 설계는 게스트에게 "서사는 **제목만**"
  (01-space-model)인데, C2는 게스트에게 `issues: []`(제목조차 안 내려감)로 확정돼 있다.
  프로토타입은 현재 제목+잠금 카드를 보여준다. → **msalt와 결정 필요**: 계약을 제목
  수준으로 완화하거나, 설계·프로토타입을 빈 배열 기준으로 수정.
