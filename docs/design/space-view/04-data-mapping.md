...
### 프레즌스 (제안) — 씬 출석부·방문 카운트

프레즌스 설계 확정(#12, [08-room-presence](../collab-space/08-room-presence.md))으로
"누가 지금 이 방에 있나"가 서버 상태가 됐다. 08 §7이 시각화 담당에게 넘긴 협의 항목
(C2 응답 형태)에 대한 space-view 쪽 제안:

**별도 경량 endpoint — `GET /spaces/{space_id}/presence`**

| 대안 | 판정 |
|---|---|
| 방 상세에 필드 추가 | 상세 응답(이슈·지식)은 무겁고 갱신이 느린데 프레즌스는 휘발성·고빈도 — 폴링 주기가 안 맞음 |
| **별도 조회 (제안)** | 씬이 짧은 주기로 이것만 폴링. 휘발 저장소 분리(08 §2)와 저장·조회 경계가 일치 |
...
// 제안 응답 형태
{
  ...
  "occupants": [
    { "agent_id": "a-17", "kind": "member", "since": "2026-07-15T08:12:00Z" },
    { "agent_id": "dp-03", "kind": "guest", "home_space_id": "data-platform",
      "since": "2026-07-15T09:21:00Z" }
  ]
}
```
...
| 필드 | 번역 | 화면 요소 |
|---|---|---|
| `occupants[]` ∩ 상세 `agents[]` | 공간 | 책상에 캐릭터. 멤버인데 부재 = 빈 책상 + 명판 |
| `kind: guest` + `home_space_id` | 공간 | 문가 방문자 캐릭터 · "OO팀에서 방문 중" 라벨 |
| `since` | 공간 | (P2) 체류 시간·파도타기 연출 재료 |

**tier 규칙 (권한 모델 소유자 = 시각화, 결정)** — 프레즌스는 공간 번역(L1)이라 전 tier
공개. 신원 필드는 `agent_id`만 내려주고 이름·역할은 방 상세 `agents[]`와 클라이언트에서
조인한다 — 게스트 응답의 `agents[]`가 이미 트리밍돼 있으므로 프레즌스에 별도 트리밍이
필요 없다(조인 안 되는 타팀 방문자는 `home_space_id` 라벨만 표시).

**파생 효과** (계약 소유자 전달 사항):

- `GET /spaces`의 `members_online` → "현재 체크인 수"로 재정의 (엘리베이터 표시는 그대로)
- 방 상세 `visits`(TODAY/TOTAL) → 게스트 체크인 기록이 원천 — 클라이언트 연출이 아니라 서버 집계
- 강제소환 등 프레즌스 이벤트 규격(08 §7 미결)은 P2 파도타기와 함께 후속 협의
