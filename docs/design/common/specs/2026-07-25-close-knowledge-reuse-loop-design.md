...
- **질의(query)**: 결정론적으로 파생한다. R8은 `format!("[a-mate] MCP '{server}'")`처럼
  렌더 제목의 식별 부분을 쓴다. LLM 무개입(정밀도의 선).
- **동일 판정**: 검색 결과 중 **제목이 렌더 제목과 완전히 일치**하는 페이지. 렌더가 결정론이라
  같은 룰·같은 대상이면 제목이 같다. 느슨한 유사도 매칭은 쓰지 않는다(오인용 방지).
