...
v1은 [Pillar 3의 데이터 계약](../frontend-viz/03-architecture.md)과 대조하기 전에 확정했고, **대조해보니 v1이 틀렸다.**
...
> — [frontend-viz 01-product §5](../frontend-viz/01-product.md)
...
`summary`·`status_line`·`highlight`·`chain[].label`은 **완성된 한국어 문장으로 내려간다.**
프론트가 조립하면 이벤트 타입이 늘 때마다 프론트를 고쳐야 한다.

### 5.2 추정치는 정직하게

`est_saved_tokens`·`tokens_saved_est`는 **추정**이다. 프론트는 반드시 `~`를 붙인다.
