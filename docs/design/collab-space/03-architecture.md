...
| **②** | **Pillar 3(시각화)이 Graph DB를 직접 조회** | 내 그래프 스키마가 곧 **Pillar 3의 공개 API**가 된다. 노드/엣지 하나 바꾸려면 매번 프론트 담당자 허락이 필요하고, 스키마가 굳어 진화가 멈춘다. | Pillar 3은 **내가 제공하는 읽기 전용 REST API만** 사용. **DB 커넥션 스트링은 절대 공유하지 않는다** (C2) |
...
│         ┌─────────────┼─────────────┐                         │
│         ▼             ▼             ▼                         │
│   [VectorStore]  [GraphStore]  [LLMClient]  ← 전부 포트(인터페이스) │
└─────────┬─────────────┬─────────────┬─────────────────────────┘
          ▼             ▼             ▼ C3: OpenAI 호환
      ChromaDB?      Neo4j?      LiteLLM 게이트웨이
                                  ★공용 인프라 — 내 소유 아님
                                        │
                              ┌─────────┼─────────┐
...
Vector DB도 Graph DB도 **아직 미확정**이다. 확정될 때까지 개발을 못 하면 안 된다.
→ Core는 **구현체가 아니라 인터페이스(포트)에만 의존**한다.

```
core/          ← 도메인 로직. 외부 라이브러리 import 금지. 인터페이스만 안다.
  ports.py       VectorStore / GraphStore / LLMClient  (추상)
...
| 실익 | 내용 |
|---|---|
| **DB 결정을 미룰 수 있다** | NetworkX로 시작해 Neo4j로 갈아타도 `core`는 **한 줄도 안 바뀐다** |
...
| 인메모리로 짠 로직이 **실제 Graph DB에서 성능이 안 나옴** | 실 DB 교체 단계에서 **실데이터 규모로 조기 검증** |
