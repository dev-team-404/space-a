...
> ⚠️ **현재는 설계 단계다.** 코드는 아직 없고, 이 문서는 아이데이션의 결과물이다.
> 확정된 결정은 [`../../adr/`](../../adr/)로 옮긴다.
...
- **어떻게**: **MCP 서버**로 노출해 Claude Code·Cursor·Claude Desktop 등 어떤 클라이언트든 설정 한 줄로 연결.
  검색은 **Vector + Graph 하이브리드** — 유사 문서를 벡터로 찾고, 그에 연결된 도구·에이전트·해결책의
  인과관계를 그래프로 추적해 **정제된 관계망 JSON만** 프롬프트에 주입한다.
- **왜 다른가**: Mem0·Letta가 *개인* 에이전트의 기억이라면 이쪽은 **조직의 집단 지성**.
  AutoGen·CrewAI가 모였다 흩어지는 *런타임* 협업이라면 이쪽은 **상시 축적되는 플랫폼**.
  Dify·Khoj가 *사람*을 위한 RAG GUI라면 이쪽은 **Agent-First 스키마 + 프로토콜**.
...
1. **MCP 서버로 만든다** (개별 Skill 스크립트 배포가 아니라) — 제로 세팅·중앙 보안·플러그앤플레이. → [01](01-product.md#5-mcp-vs-agent-skills--왜-mcp인가)
2. **GPU 클러스터는 내 것이 아니다** — Pillar 1도 쓰는 공용 인프라. 나는 소비자일 뿐. → [03](03-architecture.md#5-공용-인프라--rtx-5070-클러스터)
3. **DB 선택은 미룬다** — Core는 포트(인터페이스)에만 의존하므로, 인메모리로 시작해 나중에 갈아끼운다. → [03](03-architecture.md#4-내부-구조--포트와-어댑터)
