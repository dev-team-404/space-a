...
- **a-mate → a-hub (코칭 지식 공유).** a-mate가 규칙 엔진으로 찾은 **유의미한 코칭 발견**(예: 미사용 always-on MCP)을 스캔 후 자동으로 a-hub에 이슈→해결 흐름으로 발행한다 — "개인의 시행착오가 조직의 자산이 된다"의 구현. 무엇을·언제·어떻게 보내는지는 [설계 스펙](../design/overview-mentor/specs/2026-07-18-hub-knowledge-sharing-design.md) 참고. 환경변수(`SPACE_A_HUB_URL` 등) 미설정이면 조용히 꺼진다(프라이버시 기본 = 로컬).
