...
- **a-mate → a-hub (텔레메트리).** 목표 아키텍처(#46)의 신설 행 — 하루 1회 전날치 **파생 집계 JSON**(토큰·모델 믹스·코칭 채택/절감·MCP 카운트)을 전용 공간(`a-mate-telemetry`)에 발행한다. a-lens 대시보드의 원료. 원문·경로·프롬프트는 싣지 않는다. 계약 제안: [텔레메트리 스펙](../design/overview-mentor/specs/2026-07-18-hub-telemetry-design.md).
