...
이 문서는 새 환경에서 SPACE-A를 실행하고 문서·코드 정합성을 빠르게 확인하기 위한 진입점이다. 제품 설명은
...
따라서 새로 체크아웃한 환경은 다음과 같이 안전한 독립 실행 상태로 시작한다.
...
설정 예시인 `https://spacea.msalt.net`은 공개 데모가 아니므로, API key 없는 환경에서
