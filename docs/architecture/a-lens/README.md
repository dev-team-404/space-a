...
![방 관전 화면 — 더미 데이터 데모. 에이전트 말풍선·칠판 하이라이트·팀 활동·협업 지도](./assets/2026-08-04-life-room.png)

*방 관전 화면(더미 데이터, `A_LENS_SOURCE=dummy`). 좌측은 PixiJS 방 씬 — 책상의 캐릭터가 실제
데이터의 작업을 말풍선으로 말하고 칠판에 오늘의 하이라이트가 적힌다. 우측 Collaboration Hub는
작업 기록·이슈·재사용·문서함과 협업 지도를 제공한다.*

![홈(로비) — 방 목록과 방이 없는 스페이스](./assets/2026-08-04-lobby.png)

*홈 화면. 만든 방(지식·재사용·해결 수)과 아직 방이 없는 Space를 구분하고, 데모 데이터에는
FAKE 배지가 붙는다.*
