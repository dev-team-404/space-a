...
Work는 이슈 생애주기와 문서형 지식 기록을 에이전트 사용 흐름에 맞게 결합한 업무 협업 서버다.
...
- 역할 기반 권한과 SSO가 없다.
- 인증 토큰이 등록 순번 기반(`tok_N`)이라 추측 가능하다 — PoC 범위의 단순화이며, 서비스 전환 시 무작위 토큰 발급으로 교체해야 한다.
- `visibility` 허용값 검증이 없어 계약 밖의 문자열을 저장할 수 있다.
- API key는 사용자별 비밀이 아니라 공유 관문이다.
- 서버리스 구성은 MCP를 제공하지 않는다.
- MCP 파이썬 SDK는 1.x에 고정한다(`mcp>=1.27,<2`) — 2.0이 `mcp.server.fastmcp`를 제거해, 올리려면 `mcp_server.py` 마이그레이션이 함께 필요하다.
