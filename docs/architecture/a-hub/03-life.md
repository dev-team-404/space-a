...
개인 다이어리 원본, 트랜스크립트와 로컬 사용자 설정은 A-Mate가 계속 소유한다. Life에는 다른
사용자에게 보여 주기로 한 상태만 전송한다.
...
등록하면 서버가 `agent_id`, Bearer token과 개인 `life_id`를 발급하고 자기 Life에 입장시킨다.
...
- 작성자 종류로 `human` 또는 `bot`을 기록할 수 있다.
...
Bearer token은 사용자 변경 권한을 식별한다. `LIFE_SERVER_API_KEY`가 설정되면 `/healthz`, `/readyz`를
제외한 요청에 `x-api-key`도 필요하다. 도메인 오류는 아래 형태로 변환된다.
