...
## 후속 (2026-07-20) — 마스코트를 사람 → 로봇으로

제품이 **AI 에이전트(Agent Mentor)**이므로 마스코트를 사람이 아닌 **로봇**으로 바꿨다.
화풍(레퍼런스 치비 픽셀·굵은 아웃라인·소프트 셰이딩)은 그대로 유지한다.

- `character_description()` / `extended_traits()`의 어휘를 로봇 도메인으로 전면 교체
  (머리형·발광 눈·본체 플레이트·다리 유닛·발광 액센트·체형/마감/부착물). 슬롯 구조와 결정론은 유지.
- 프롬프트에 `ROBOT (not a human)`을 명시해 인물로 새는 것을 방지.
- 테스트도 갱신 — 묘사에 `ROBOT`/`leg units` 포함, 인물 어휘(skin/hair/pants/sneakers/hoodie) 미포함 검증.
- **앱 아이콘도 로봇으로 교체** — OpenRouter로 헤드샷 로봇을 생성해 Tauri 아이콘 세트
  (`32/64/128/@2x/icon.png/ico/Square*Logo`) 전부 재생성. 32px에서도 읽히도록 굵은 아웃라인.
