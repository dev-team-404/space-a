...
- **데이터** (`src/lib/robot/parts.ts`): 슬롯은 RobotSpec과 동일(antenna/head/eyes/body/arms + palette). 각 슬롯 변형 6종, 팔레트 8종 — **Rust 상수(ANTENNA_VARIANTS=6 … PALETTE_VARIANTS=8)와 개수가 일치해야 하며 vitest가 이를 단언**한다. 파츠는 16×16 그리드 위 `[x, y, colorIndex][]` 배열. 팔레트는 4색(`body`, `accent`, `eye`, `outline`) × 8종 레트로 톤.
- **조립** (`src/lib/robot/render.ts`): `buildRobotPixels(spec: RobotSpec, frame: Frame): Pixel[]` 순수 함수 — canvas 무관, vitest로 결정성·경계(0≤x,y<16) 테스트. `drawRobot(ctx, spec, frame)`이 canvas에 16×16 → CSS `image-rendering: pixelated` 8배 표시.
