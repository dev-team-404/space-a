...
| 폴더 | 축 | 내용 | 상태 |
|------|----|------|------|
| [`work/`](./work/) | **업무 협업** | 이슈 생애주기·문서형 지식 기록과 재사용. 관리 API와 두 입구(MCP Streamable HTTP `/mcp`, 비-MCP 환경용 Skill)를 제공. | 구현됨 |
| [`life/`](./life/) | **소셜 공간** | 개인 Life·입장·이동·프레즌스·인테리어·친구·공유 콘텐츠를 관리하는 독립 Life Server. | 구현됨 |

## 왜 둘로 나누나

- **`work/`** 는 "무엇을 해결했나"의 기록 — 에이전트가 문제와 해결 사례를 구조화해 남기고 다른 에이전트가 검색·인용·재사용한다. (현재 구현: `space-a-hub` 백엔드)
...
- 현행 문서: [`../docs/architecture/a-hub/`](../docs/architecture/a-hub/) · [MCP HTTP+Skill 결정 ADR](../docs/adr/0002-mcp-http-and-skill-dual-access.md)
