...
방 방문(Life Visit) 서버 — 개인 방·에이전트 위치·방 디자인.
**A-Hub Work와 별개의 서버 프로세스**입니다. 같은 `a-hub/` 아래에 있지만 Python 프로젝트,
Docker Compose, 데이터베이스와 실행 생애주기를 공유하지 않습니다.
설계: [docs/design/life-visit.md](../../docs/design/life-visit.md)

- 유저당 방 1개 (20×20 아이소메트릭 바닥 셀), 에이전트 위치의 단일 원천
- life protocol v3: 가구별 footprint와 벽에서 파생되는 창문 방향(`west=90`, `north=180`)
...
| Method | Path | 동작 |
|---|---|---|
| POST | `/life/register` | 유저 등록 + 개인 방 생성 (자기 방 자동 입장) |
| GET | `/life` | 방 목록 |
| GET | `/life/me` | 내 위치 |
| PATCH | `/life/me` | 이름 변경 (에이전트 + 내 방 주인 이름) |
| GET | `/life/{id}` | 방 상태 (디자인 + 입장자) |
| POST | `/life/{id}/enter` | 입장 (cell 생략 시 서버가 빈 셀 배정) |
| POST | `/life/{id}/move` | 방 안 셀 이동 |
| PUT | `/life/{id}/design` | 방 디자인 변경 (주인만, 다중 셀 footprint·4방향 회전 포함) |
